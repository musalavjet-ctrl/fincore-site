  ...
  return (
    <section id="industries" className="py-12 sm:py-16 bg-white">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-[#1d1d1f] text-center mb-4">{t('industries.title')}</h2>
        <p className="text-[#86868b] text-center mb-8 max-w-xl mx-auto">
          {t('industries.subtitle')}
        </p>
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {industries.map(ind => (
            <div key={ind.key} className="bg-[#f5f5f7] rounded-2xl p-4 sm:p-6 hover:shadow-md transition-shadow">
              ...
              <h3 className="text-[13px] sm:text-sm font-semibold text-[#1d1d1f] mb-1">{t(`industries.${ind.key}.title`)}</h3>
              <p className="text-xs sm:text-sm text-[#86868b] leading-relaxed">{t(`industries.${ind.key}.desc`)}</p>
            </div>
          ))}
        </div>
      </div>
